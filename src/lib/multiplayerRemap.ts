...
export function remapSimStateForClient(state: SimState, role: 'host' | 'guest'): SimState {
  ...
  const combatMoraleState = new Map(
    Array.from(state.combatMoraleState.entries()).map(([key, v]) => {
      const mappedOwner = mp(v.ownerId);
      const sep = key.lastIndexOf(':');
      const hexPart = sep >= 0 ? key.slice(0, sep) : key;
      return [`${hexPart}:${mappedOwner}`, { ...v, ownerId: mappedOwner }] as const;
    }),
  );
