...
export default function MapController({ target, applyTargetUpdates = true }: MapControllerProps) {
  ...
  useEffect(() => {
    if (target && controlsRef.current) {
      const nextTargetKey = target.join('|');
      if (!applyTargetUpdates && appliedInitialTargetRef.current && lastAppliedTargetRef.current === nextTargetKey) {
        return;
      }
      controlsRef.current.target.set(...target);
      controlsRef.current.update();
      appliedInitialTargetRef.current = true;
      lastAppliedTargetRef.current = nextTargetKey;
    }
