...
Production deploys require deployment-specific `SITE_PASSWORD` and `COOKIE_SECRET` values; the app fails closed if either is missing. Locally, leave both unset for an open dev server, or set both in [`.env.local`](.env.example) to test `/login`.
