...
- **Sim hot-path optimization audit (Mar 2026):** Per-season phase timing added (env `SIM_DEBUG_TIMING=1`). Optimizations: (1) `diagnosticsLevel: 'minimal'` (env `SIM_DIAG=minimal`) skips expensive telemetry in stepSimulation while keeping totalKills, killsByAi1/2, totalStarvationAbort for scoring. (2) gameCore: unitById map for move-target lookups; (3) military upkeepTick: clusterByKey and heroLogisticianByHex indexes to avoid repeated find(). Determinism preserved (same seed → same outcome). Benchmark: `npm run benchmark-sim` (optional `SIM_DIAG=minimal`). See [SIM_BENCHMARK_SUMMARY.md](./SIM_BENCHMARK_SUMMARY.md).
- **4-player incentives and win conditions:** Conquest victory and time's-up are both implemented for 4-bot in the store: conquest = exactly one AI has cities (`whoHasCities.length === 1`); time's up = rank all four by cities then pop, declare winner. In `src/lib/ai.ts`, `enemyCities = cities.filter(c => c.ownerId !== aiPlayerId)` — so in 4-player each AI sees all 3 opponents' cities. Targeting uses a single sorted list (weakest first by pop + defenders), primary target + up to 3 alternates via `nearestTargetDistanceRatio`; scouts pick nearest enemy. No code assumes exactly two players. Headless sim and training (`gameCore`) are 2-player only; 4-bot uses the same params; behavior is consistent but not specifically evolved for 4-player.
...
- [Sim benchmark summary](./SIM_BENCHMARK_SUMMARY.md) — hot-path audit, phase timing, benchmark table
