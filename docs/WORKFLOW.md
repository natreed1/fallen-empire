...
- **Food controls (execution-level, hard):** **maxSustainableMilitary** is now a true execution-level cap: `sustainableMilitaryMultiplier` can only reduce the cap (e.g. 0.8 = recruit up to 80% of food-sustainable military), never exceed it. **foodBufferThreshold** is now hard: no military or builder recruits when surplus &lt; threshold (previously soft: up to 1 recruit allowed). **Tier instability:** Soft food controls let agents overshoot into starvation; counting per-side “starvation lock” games made tier scoring noisy. Replaced with **totalStarvationGame**: when *both* sides are irrecoverable (all military starving + zero food), game aborts and both AIs get a large penalty; telemetry uses `totalStarvationGames` / `totalStarvationRate` and rollback uses `maxTotalStarvationRateTrigger`. This should reduce tier system instability from ambiguous “one side locked” outcomes.
...
- **AI training system overhaul (2026-04-03):** The headless simulation (`gameCore.ts`) now runs **contested zone payouts**, **scroll search progress**, and **commander creation/sync** — systems that were live-game only. `planAiTurn` gained 3 new action types: **commander field assignment** (attach idle city-defense commanders to moving army stacks), **scroll attachment** (equip discovered scrolls onto carriers), and **university task selection** (set builder workforce priorities per city). AI also now **diverts units toward contested zone hexes** for gold/iron income and **positions units on special terrain** for scroll discovery. 8 new evolvable params added to `AiParams` / `aiParamsSchema.ts`: `contestedZoneCommitShare`, `contestedZoneMinSurplusMilitary`, `commanderFieldAssignRate`, `commanderMinArmySize`, `scrollTerrainPriority`, `scrollTerrainMaxDivert`, `universityIronMinePref`, `universityCityDefenseThreshold`. Diagnostics track contested zone wins, scrolls discovered, and commander field assignments per side. Champion `ai-params.json` seeded with sensible defaults for new params. **Next step:** run `npm run train-ai` to evolve optimal values for the new params alongside existing ones.

- **Army combat control (2026-08-15):** Stance existed in combat math but was unreachable in the live HUD (`setStance` had no buttons; `OperationalArmy.stance` was never applied). Cross-hex volleys only ran for aggressive/skirmish, so Hold/Defend archers did nothing at range; flanking only counted aggressive neighbors; aggressive’s defense penalty was skipped. **Now:** army / hex / battle / Army-panel scope all set stance in one click; Withdraw is the real 2s retreat (battle report “Retreat” was actually March). Recruits from a training stack linked to a field army inherit that army’s id and stance. Hold/Defend shoot in range but do not chase; only Aggro pursues when shot.
- **Siege vs closed walls (2026-08-15):** Playtest against a ring-1 walled AI keep: infantry cannot walk onto intact wall hexes; the old siege rally picked a *walled* neighbor so the camp never arrived; AI always ordered a charge to the city center and bounced. **Now:** siege camp prefers a ring-1 gap, else ring-2 outside the walls (engines still reach); assault is refused until a section falls; AI marches to that camp instead of the keep. Trebuchet playtest: camp → break a section → infantry can enter.
- **Siege play vs AI (2026-08-15, pass 2):** Assault/Direct still charged a closed keep and bounced. AI stacked every unit on the nearest ring-2 hex, so they never encircled. `siegingCityId` and the siege panel said “starve” but food still pooled empire-wide (`computeTradeClusters` does not exist). **Now:** all three attack styles camp when there is no wall gap; AI spreads camps to cover unused wall approaches; a city is surrounded when every land ring-1 hex is occupied or its wall is covered from ring-2; isolated cities eat only their own stores (civilian + garrison).
- **Four-system pass (2026-08-15):** Economy: `foodPriority` now reserves military upkeep when set to Feed Army. Military: capture treats intact ring-1 walls as blocking (not a wall on the center hex). Political: council `goldMult` / `productionMult` apply in tax and production. Research: leftover RP carries past tech completion instead of resetting to 0.
