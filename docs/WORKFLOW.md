...
- **AI test environments (2026-04-26):** `/test-env/[envId]` routes are deterministic sandbox screens for model-evaluation previews. `loading-screen` renders the loading shell without booting the game; `combat-risk-preview` starts the existing battle test, opens the battle report, and pauses the sim; HUD/economy/save-load/AI-planning routes boot paused observer states for immediate inspection from the LoRA arena preview links.
...
- **Research & Technology system (2026-04-15):** Added a full research and technology tree, national civilian panel, education system, politicians, and university buildings. **Technology tree:** 17 techs gating building construction, unit recruitment, and building upgrade levels. Starting techs (agriculture_1, mining_1, forestry_1, military_tactics_1, economics_1) are free; advanced techs require literacy-based research points accumulated over cycles. **Education:** National education level (1–5, upgradable with gold) and literacy (0–100 scale) drive research speed. Universities boost literacy per cycle. **Universities:** New city building that generates commanders and politicians based on specialization (Military, Economics, Research, General). Levels 1–5 with increasing graduate chance and education contribution. **Politicians:** New person type with economic traits (economist, merchant, diplomat, administrator, scholar, industrialist). Generated by universities alongside commanders. **National Council:** Four posts (Chancellor, Treasurer, Marshal, Spymaster) — assign commanders or politicians for amplified combat or economic bonuses. **UI:** "Civilian" button in TopBar → full-screen panel with Council, Education, and Research tabs. Tech tree shows unlocks, prerequisites, and progress. **Tech gates:** Building construction blocked if required tech not researched; barracks upgrade level gated by tech. **Files:** `src/types/game.ts` (types + tech tree), `src/lib/nationalCouncil.ts` (council logic + politician generation), `src/lib/researchTick.ts` (literacy + research per cycle), `src/store/useGameStore.ts` (state + actions), `src/components/ui/GameHUD.tsx` (CivilianPanel).
