...
import { mergePlans, sanitizeClientPlan } from './clientPlans.ts';
...
wss.on('connection', (socket) => {
  socket.on('message', (data) => {
    ...
    if (msg.type === 'join' && msg.roomId && msg.role) {
      ...
      if (msg.role === 'host') {
        if (room.clients.size >= 2 || Array.from(room.clients.values()).some(c => c.role === 'host')) {
    ...
    if (msg.type === 'plan' && msg.plan) {
      ...
      found.pending[meta.playerId] = mergePlans(
        cur,
        sanitizeClientPlan(msg.plan, found.state, meta.playerId),
      );
      return;
    }
  });
