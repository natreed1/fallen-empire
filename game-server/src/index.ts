...
import { mergeClientPlan, sanitizeClientPlanPatch } from './clientPlans.ts';
...
wss.on('connection', (socket) => {
  socket.on('message', (data) => {
    ...
    if (msg.type === 'plan' && msg.plan) {
      ...
      found.pending[meta.playerId] = mergeClientPlan(
        cur,
        sanitizeClientPlanPatch(found.state, meta.playerId, msg.plan),
      );
      return;
    }
  });
