...
import { mergeClientPlanPatch } from './clientPlans.ts';
...
wss.on('connection', (socket) => {
  socket.on('message', (data) => {
    let msg: { type?: string; roomId?: string; role?: string; plan?: Partial<AiActions> };
    try {
    ...
    if (msg.type === 'join' && msg.roomId && msg.role) {
      ...
      if (Array.from(room.clients.values()).some(client => client.role === msg.role)) {
        socket.send(JSON.stringify({ type: 'error', message: `${msg.role} slot is already occupied.` }));
        return;
      }
    ...
    if (msg.type === 'plan' && msg.plan) {
      ...
      found.pending[meta.playerId] = mergeClientPlanPatch(cur, msg.plan, found.state, meta.playerId);
      return;
    }
  });
