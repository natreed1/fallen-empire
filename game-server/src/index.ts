...
wss.on('connection', (socket) => {
  socket.on('message', (data) => {
    ...
    if (msg.type === 'join' && msg.roomId && msg.role) {
      ...
      const admission = validateRoomJoin(room.clients.values(), msg.role, room.state != null);
      if (!admission.ok) {
        socket.send(JSON.stringify({ type: 'error', message: admission.message }));
        return;
      }
    ...
    if (msg.type === 'plan' && msg.plan) {
      ...
      found.pending[meta.playerId] = mergePlayerPlan(cur, msg.plan);
      return;
    }
  });
