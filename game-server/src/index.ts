...
wss.on('connection', (socket) => {
  socket.on('message', (data) => {
    ...
    if (msg.type === 'plan' && msg.plan) {
      ...
      if (!found.state) {
        socket.send(JSON.stringify({ type: 'error', message: 'Room is not ready.' }));
        return;
      }
      const cur = found.pending[meta.playerId] ?? emptyAiActions();
      const sanitized = sanitizeClientPlan(msg.plan, meta.playerId, found.state.units, found.state.tiles);
      found.pending[meta.playerId] = mergePlan(cur, sanitized);
      return;
    }
  });
