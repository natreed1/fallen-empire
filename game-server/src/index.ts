...
function isValidMoveAction(value: unknown): value is AiMoveAction {
  if (!value || typeof value !== 'object') return false;
  const m = value as Partial<AiMoveAction>;
  return (
...
function sanitizeHumanPlan(patch: unknown, playerId: string, state: SimState): AiActions {
  ...
  if (!Array.isArray(moveTargets)) return safe;

  const ownedUnits = new Set(
    state.units
      .filter(u => u.ownerId === playerId && u.hp > 0 && u.status !== 'fighting')
      .map(u => u.id),
  );

  safe.moveTargets = moveTargets.filter((m): m is AiMoveAction => {
    if (!isValidMoveAction(m)) return false;
    return ownedUnits.has(m.unitId) && state.tiles.has(tileKey(m.toQ, m.toR));
  });
  return safe;
}
  ...
  socket.on('message', (data) => {
    ...
    if (msg.type === 'join' && msg.roomId && msg.role) {
      ...
      if (Array.from(room.clients.values()).some(c => c.role === msg.role)) {
        socket.send(JSON.stringify({ type: 'error', message: `${msg.role === 'host' ? 'Host' : 'Guest'} slot is already occupied.` }));
        return;
      }
    ...
    if (msg.type === 'plan' && msg.plan) {
      ...
      found.pending[meta.playerId] = mergePlan(cur, sanitizeHumanPlan(msg.plan, meta.playerId, found.state));
      return;
    }
  });
